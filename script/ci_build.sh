...
#
# Actions: dependencies|generate|generate-detect-thirdparty|build|build-configurations|
#          build-swig|install|test|test-configurations|cstest|
#          cstest-configurations|apk|publish-to-itch|release-mobile-artifacts|
...
then
    ...
    echo "  build-configurations      Build all selected configurations"
    echo "  build-swig                Build host SWIG tool for cross builds"
    echo "  install                   Install build artifacts"
    echo "  test                      Run native tests"
    echo "  test-configurations       Run native tests for all selected configurations"
    echo "  cstest                    Run C# tests"
    echo "  cstest-configurations     Run C# tests for all selected configurations"
    ...
    echo ""
    echo "Options:"
    echo "  --build-type TYPE         Configured short build type name"
...
default_build_types='{"dbg":"Debug","rel":"RelWithDebInfo"}'
...
while IFS=$'\t' read -r build_type build_type_name; do
    ...
    if [[ "$ci_platform" != 'android' ]]; then
        types["$build_type"]="$build_type_name"
    fi
done < <(python3 -c '
import json
import sys
...
try:
    build_types = json.loads(sys.argv[1])
except json.JSONDecodeError as error:
...
for short_name, build_type_name in build_types.items():
    if (
        ...
        or not short_name
        or not isinstance(build_type_name, str)
        or not build_type_name
...
if [[ "$ci_platform" == 'android' ]]; then
    for build_type in "${configured_build_type_ids[@]}"; do
        if [[ -z "${types[$build_type]:-}" ]]; then
            echo "Error: Android build type '$build_type' has no CMake configuration mapping"
            exit 1
        fi
    done
fi
...
prepare-project-cmake-args() {
    ...
    configuration_types=$(get-cmake-configuration-types)
    prepare-project-search-paths
    ...
    project_cmake_args=(
        ...
        -DCMAKE_CONFIGURATION_TYPES="$configuration_types"
...
build-project-configurations() {
    local build_dir=$1
    local build_types=("${configured_build_type_ids[@]}")
...
function action-build() {
    # Arguments: --build-type <short-name>, -- <extra_args...>
...
function action-build-configurations() {
    # Arguments: none
    local old_build_type="${arg_build_type:-}"
    local build_type=''
    for build_type in "${configured_build_type_ids[@]}"; do
        echo "Building configuration ${configured_build_type_names[$build_type]}"
        if [[ "$ci_platform" == 'android' ]]; then
            run-android-gradle-target \
                "$ci_source_dir/android" \
                "${configured_build_type_names[$build_type]}" || return 1
        else
            arg_build_type="$build_type"
            action-build || return 1
        fi
    done
    arg_build_type="$old_build_type"
}
...
function action-install() {
    # Arguments: --build-type <short-name>, --sdk-layout <bool>, --prefix <dir>, -- <extra_cmake_args...>
...
function action-cstest-configurations() {
    # Arguments: none
    local old_build_type="${arg_build_type:-}"
    local build_type=''
    for build_type in "${configured_build_type_ids[@]}"; do
        arg_build_type="$build_type"
        action-cstest || return 1
    done
    arg_build_type="$old_build_type"
}
...
function action-release-mobile-artifacts() {
    ...
    # Prefer the release configuration when the grouped plan produced more than one
    # mobile artifact, then fall back to the first matching artifact.
    local artifact=''
    if [[ -n "${configured_build_type_names[rel]:-}" ]]; then
        case "$ci_platform" in
            android)
                artifact=$(find . -iname '*release*.apk' -type f | head -n 1)
                ;;
            ios)
                artifact=$(find . -path "*/${types[rel]}/*.app" -type d | head -n 1)
                ;;
        esac
    fi
    if [[ -z "$artifact" ]]; then
        artifact=$(find . -name "$pattern" $([[ "$ci_platform" == "ios" ]] && echo "-type d") | head -n 1)
    fi
    if [ -n "$artifact" ];
    then
...
function action-test-project() {
    ...
    then
        ...
        for build_type in "${configured_build_type_ids[@]}"; do
            cmake --build \
                "$build_dir" \
                --config "${types[$build_type]}" || return 1
        done
    else
...
function action-build-project() {
    ...
    #            --install-dir <dir> (optional), --build-type <short-name> (optional),
    #            -- <extra_cmake_args...>
...
function action-build-android-project() {
    ...
    if [[ -n "${arg_gradle_task:-}" ]]; then
        run-gradle-task "$android_dir" "$arg_gradle_task"
        return
    fi
